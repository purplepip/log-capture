...
class LogCaptureContextFactory {
  ...
  static {
    ...
    try {
      Class<LogCaptorContext> logCaptureClazz =
          (Class<LogCaptorContext>) Class.forName(logCaptureClassName);
      LOG_CAPTURE_CONTEXT_CLASSES.put(loggerFactoryClassName, logCaptureClazz);
    } catch (ClassNotFoundException e) {
      LOG.warn(
          "Cannot find class {} for logger factory {}",
          logCaptureClassName,
  ...
  LogCaptorContext create() {
    return create(LoggerFactory.getILoggerFactory().getClass().getName());
  }

  LogCaptorContext create(String loggerFactoryClassName) {
    Class<LogCaptorContext> logCaptureClazz =
        LOG_CAPTURE_CONTEXT_CLASSES.get(loggerFactoryClassName);
    ...
    try {
      ...
      throw new LogCaptorException(
          "Cannot create log captor class "
              + logCaptureClazz
              + " for logger factory "
              + loggerFactoryClassName,
          e);
